...
__thread const char* errstr = "successfull";

__private void tig_error_set(void){
...
const char* tig_strerror(void){
	return errstr;
}

int tig_clone(const char* url, const char* path, tigProgress_f progress, void* userdata){
	dbg_info("url:%s", url);
...
int tig_pull(int* nothing, const char* path, const char* origin, tigProgress_f progress, void* userdata){
	int err;
	...
	if( (err = git_repository_open(&repo, path)) || repo == NULL ) goto ONERR;
	if( (err = git_remote_lookup(&remote, repo, origin)) ) goto ONERR;
	...
	if( (err = git_remote_fetch(remote, NULL, &fopt, NULL)) ) goto ONERR;
